...
function CellContent({ value, isRecommended, isPrice }: { value: string; isRecommended: boolean; isPrice: boolean }) {
  // Check/X rendering
  if (value === '✓') return <span className="text-green-400 text-xl font-bold">✓</span>;
  if (value === '✗') return <span className="text-red-400 text-xl font-bold">✗</span>;

  // Starts with ✓ or ✗
  if (value.startsWith('✓')) return <span><span className="text-green-400 font-bold">✓</span><span className="text-gray-300 text-sm"> {value.slice(1).trim()}</span></span>;
              ...
              {columns.map((col, i) => (
                <th
                  ...
                  className={`p-4 text-center text-sm font-bold border-b border-white/10 ${
                    i === recommendedIndex
                      ? 'bg-green-500/[0.08] text-green-400 border-x border-green-500/20'
                      : 'text-gray-200'
                  }`}
                >
                  {i === recommendedIndex && (
                    <span className="block text-[10px] uppercase tracking-widest text-green-500 mb-1">★ Our Pick</span>
                  )}
                  {col}
                </th>
              ))}
                ...
                {row.values.map((val, ci) => (
                  <td
                    ...
                    className={`p-4 text-center border-b border-white/5 ${
                      ci === recommendedIndex ? 'bg-green-500/[0.05] border-x border-green-500/10' : ''
                    }`}
                  >
